...
if (typeof gemius_cookie == "undefined") {
	gemius_cookie = {
		...
		get : function(name) {
			var ret = {primary: '-TURNEDOFF', secondary: '-TURNEDOFF', sec_state: "unknown", err: ""};
			try {
		...
		get_on_state : function(name,sec_state) {
			...
			if (sec_state=="unknown" || cookie.sec_state==sec_state) {
				...
				return {primary: cookie.secondary, secondary: '-TURNEDOFF', sec_state: "sec", err: cookie.err};
			} else {
				return {primary: '-TURNEDOFF', secondary: '-TURNEDOFF', sec_state: "unknown", err: cookie.err};
			}
		},
...
if (typeof GemiusHitcolConnector == "undefined") {
	GemiusHitcolConnector = function(is_gde) {
		...
		if (this.use_globals) {
			...
			this.hc = (typeof gemius_hitcollector === 'string')?gemius_hitcollector:(typeof pp_gemius_hitcollector === 'string')?pp_gemius_hitcollector:'gaua.hit.gemius.pl';
...
var check_same_origin = function(ref, page_location) {
	...
	return null;
		...
		this.get_referrer_override = function() {
			...
			return (frametype == 0 || frametype == 1 || (frametype == 3 && this.ref.topref==0)) ? this.ref.override : null;
		};
		var _get_referrer = function() {
			return this.get_referrer_override() || this.ref.ref;
		};
		_get_referrer = this.bind(_get_referrer);
		var _PRERENDER_API_S = 'prerendering';
		var _PRERENDER_API_C = 'prerenderingchange';
		var _is_prerender = function() {
			if (_PRERENDER_API_S in document) {
				return document[_PRERENDER_API_S];
			}
			if (this.visapi_s) {
				return document[this.visapi_s] === 'prerender';
			}
			return false;
		}
		_is_prerender = this.bind(_is_prerender);
		...
		var _on_page_activation = [];
		var _run_after_prerender = function(fn) {
			if (_is_prerender()) {
				_on_page_activation.unshift(fn);
			} else {
				fn();
			}
		}
		this.prerender_changed = function() {
			if (this.waiting_on_prerender && !_is_prerender()) {
				this.waiting_on_prerender = 0;
				setTimeout(this.bind(this.latehits),100);
				while (_on_page_activation.length) {
					_on_page_activation.pop()();
				}
			}
		};
		...
		var _register_cmp_timeout = function() {
			this.timeouts.cmpto = this.set_timeout(this.bind(this.consentto),this.cmp_timeout);
		}
		_register_cmp_timeout = this.bind(_register_cmp_timeout);
		var _register_timer_interval = function() {
		...
		this.setfpcookie = function() {
			this.fpdbg = gemius_cookie.set(this.fpcookie_name, this.fpdata, this.cookie_expire, this.fpcdomain, this.fpdata_sec_state, this.fpcookie_secondary_needs_reset());
			if (this.is_gde) {
				this.sync_gde_params();
			}
		};
		...
		this.reset_fpcookie_secondary_if_needed = function() {
			if (this.fpcookie_secondary_needs_reset()) {
				this.fpdbg = gemius_cookie.set(this.fpcookie_name, this.fpdata, this.cookie_expire, this.fpcdomain, 'sec', true);
			}
		};
		...
		this.addframe = function(ssl,file,args,rcvfn) {
			if (document.body) {
				...
				var url = 'http'+(ssl?'s':'')+'://ls.hit.gemius.pl/ls'+file+'.html'+args+LS_HOPTS;
				if (rcvfn!=null) {
		...
		this.frameto = function() {
			if (this.timeouts.fto!=null) {
				...
				if (this.lsdata=='') {
					this.ltime = 10000;
					this.lsdata = _PRERENDER_API_S in document?'-PRERENDER':'-TIMEDOUT';
		...
		var _nodata = function(data) {
			return data.charAt(0)=='-';
		}
		this.last_datareceiver = function(lsdata) {
			this.current_receiver = null;
			if (_nodata(lsdata)) {
				this.lsdata = lsdata.split('/')[0];
				this.lsdbg = lsdata.substring(this.lsdata.length+1);
			} else {
				this.lsdata = lsdata;
				this.lsdbg = "";
			}
		...
		this.second_datareceiver = function(data) {
			if (_nodata(data) || data=='') {
				this.lsdbg = data.substring(data.indexOf('/'));
...
// cmp_utils.js 
		...
		this.frametype_init = function() {
			...
			if (ref.frametype==3 && window.parent===window.top) {
				ref.topref = 1;
			}
		};
		...
		this.inner_params = function() {
			if (typeof encodeURIComponent != 'undefined') {
				...
				if (this.lsdbg) {
					inner += '|_lsd=' + this.lsdbg.substring(0,30);
				}
				if (this.fpdbg) {
					inner += '|_fpd=' + this.fpdbg.substring(0,30);
				}
	...
	return params;
		...
		this.findchints = function() {
			if (typeof navigator.userAgentData == 'object' && typeof navigator.userAgentData.getHighEntropyValues == 'function') {
				try {
					var hints = ["architecture","model","platform","platformVersion","bitness","mobile","wow64","brands","fullVersionList","formFactor"];
		...
		this.init = function(params) {
			...
			_run_after_prerender(_register_timer_interval);
			...
			try {
				if (this.dnt==0 && this.explicit_consent!==false) {
					var hasPostMessage = typeof window.postMessage != 'undefined';
					var hasLocalStorage = typeof localStorage != 'undefined' && localStorage != null;
					this.waiting_for_lsdata = (hasPostMessage && hasLocalStorage)?1:0;
					if (this.waiting_for_lsdata==0) {
						this.lsdata='-NOTSUP';
						this.lsdbg=(hasPostMessage?'':'noPostMsg')+(hasLocalStorage?'':'noLoStrg');
					}
				} else {
					this.waiting_for_lsdata = 1;
					this.lsdata = "-DNT";
			...
			if (this.dnt==0 && this.explicit_consent!==false) {
				if (this.explicit_consent===null && this.use_cmp && (typeof gemius_cmpclient == "object") && gemius_cmpclient.find_cmp()) {
					...
					if (this.cmp_timeout==Infinity) {
						...
						_run_after_prerender(_register_cmp_timeout);
					}
		...
		this.start = function() {
			...
			if (_is_prerender()) {
				this.waiting_on_prerender = 1;
				var preapi_c = _PRERENDER_API_S in document?_PRERENDER_API_C:this.visapi_c;
				this.add_event(document,preapi_c,this.bind(this.prerender_changed));
			}
			this.gen_exid();
				...
				if (!this.init_called) {
					...
					if (typeof timeout == 'number') {
						if (timeout == Infinity) {
							...
							var register_init_timeout = function() {
								this.timeouts.initto = this.set_timeout(this.bind(this.init_timeout),timeout);
							}
							_run_after_prerender(this.bind(register_init_timeout));
						}
					} else {
						gemius_init();
					}
